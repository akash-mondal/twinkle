...
export function AppsShowcase() {
  const liveApp = apps.find((a) => a.status === "live")!;
  const comingApps = apps.filter((a) => a.status === "coming");
    ...
    <section className="py-24 px-6 bg-[#09090B] relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative">
        ...
        <motion.div
          ...
          className="mb-8"
        >
          <Link href={`/apps/${liveApp.id}`} className="block group">
            <div className={`relative ${liveApp.bg} rounded-2xl p-8 md:p-12 overflow-hidden border border-white/10`}>
              {/* Decorative elements */}
              ...
              <div className="relative flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-4">
                    <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${liveApp.gradient} flex items-center justify-center shadow-lg`}>
                      <liveApp.icon className="w-7 h-7 text-white" />
                    </div>
                  ...
                  <h3 className={`text-3xl md:text-4xl font-bold ${liveApp.textColor} mb-2 font-[family-name:var(--font-syne)]`}>
                    {liveApp.name}
                  </h3>
                  <p className={`text-xl ${liveApp.textColor} opacity-80 mb-4`}>
                    {liveApp.tagline}
                  </p>
                  <p className={`${liveApp.textColor} opacity-60 max-w-md`}>
                    {liveApp.description} Create a monetized paste in seconds, share the link, and get paid in MNEE when someone unlocks it.
                  </p>
                </div>

                <div className="flex-shrink-0">
                  <div className={`inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r ${liveApp.gradient} text-white rounded-xl font-medium shadow-lg group-hover:shadow-xl group-hover:scale-105 transition-all`}>
              ...
              <div className="absolute bottom-4 right-4 md:bottom-8 md:right-8 opacity-20 font-mono text-xs hidden md:block">
                <pre className={liveApp.textColor}>
  ...
  price: "0.50 MNEE"
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
          {comingApps.map((app, i) => (
            ...
            >
              <div className={`relative ${app.bg} rounded-xl p-6 border border-white/5 opacity-70 hover:opacity-90 transition-opacity h-full`}>
                {/* Icon */}
                <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${app.gradient} flex items-center justify-center mb-4`}>
                ...
                <p className={`text-sm ${app.textColor} opacity-60 mb-2`}>
                  {app.tagline}
                </p>
                <p className={`text-xs ${app.textColor} opacity-40`}>
                  {app.description}
                </p>

                {/* Coming badge */}
                <div className="absolute top-4 right-4">
                  <span className={`text-[10px] font-medium ${app.textColor} opacity-40 uppercase tracking-wider`}>
                    Soon
                  </span>
                </div>
              </div>
